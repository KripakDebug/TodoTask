import {createTaskEntity, createTodoEntity, resolveTaskCompletion} from "./todosStore.services.js";
...
export function addTaskToList(listId, taskName) {
   ...
   const updatedTodoList = todoList.map(todoItem => {
       ...
       return {...todoItem, tasks: [...todoItem.tasks, createTaskEntity(taskName)]}
   })
...
export function updateTodoName(id, name) {
   ...
   _setTodosToStorage(todoList.map(todoListItem => {
